...
// Convergence flags
// unconverged: neither convergence nor divergence detected
// success:  residual norm <= convTol
// diverged: residual norm >= divTol
// maxits: maximum number of iterations were reached 
// true_rnrm: true residual norm > convTol
// inf_or_nan: residual norm is inf or nan
// precon_fail: application of the preconditioner failed
// matvec_fail: matrix-vector product failed
// breakdown: input matrix or preconditioner are (likely) singular
// stagnation: |xnew(i) - x(i)| <= stgTol * |xnew(i)| for all i
enum class ConvFlag { unconverged, success, diverged, maxits, 
    true_rnrm, inf_or_nan, precon_fail, matvec_fail, breakdown,
    stagnation };
...
struct GMRESParams {
    ...
    bool testTrueRnrm;   // if true, check that true residual norm of solution <= convTol
...
// throws an exeception if any parameters in p are out of range
void checkGMRESParams(const GMRESParams& p);

struct GMRESOut {
    ConvFlag flag;       // convergence flag
    index_type innerIts; // inner iteration number at which x was computed
    index_type outerIts; // outer iteration number at which x was computed
    real_type relres;    // scaled residual norm of solution
...
// Implements preconditioned, restarted GMRES with Modified
// Gram-Schmidt orthogonalization. Solves the preconditioned
// linear system A*M^{-1}*u = b, where u = M*x and
// M is the preconditioner. The solver converges at the kth
// iteration if
//     1. k < maxits * kspaceSz
//     2. ||r_k|| < divTol * ||r_0||
//     3. ||r_k|| <= max(relTol * ||b||, absTol)
// where ||.|| is the vector 2-norm and r_k is the 
// unpreconditioned residual of the kth iterate.
class GMRESSolver {
public:
    // Input/Output:
    // 1. mvec is a functor that computes a matrix-vector product.
    ...
    // 2. precon is a functor that applies the preconditioner to a vector.
    //    It must define an overload of operator() with the signature:
    //        bool operator()(const vector_type& in, vector_type& out)
    //    which returns true if successful.
    // 3. b is the rhs of the linear system.
    // 4. x contains the intial guess on input and the solution on output.
    // 5. params are the input parameters for GMRES.
    // Additional information is returned in the GMRESOut structure.

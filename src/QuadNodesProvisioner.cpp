...
namespace blitzdg {
    ...
    void QuadNodesProvisioner::splitElements(const real_matrix_type& x, const real_matrix_type& y, const real_matrix_type& field,
		...
		const index_type N = NumFacePoints - 1;
		...
		// Unpack 1D arrays storing EToV and Vertex coordinates
		index_vector_type va(totalNewElements), vb(totalNewElements), vc(totalNewElements), vd(totalNewElements);
		...
		xnew.resize(4, totalNewElements);
		ynew.resize(4, totalNewElements);
		fieldnew.resize(4, totalNewElements);

		for (index_type i=0; i < totalNewElements; ++i) {
			 index_type vai = va(i), vbi = vb(i), vci = vc(i), vdi = vd(i);
